{%  with m.onnet.user_balance_notify as notify %}
    ...
    <tbody id="top_up_settings" style="display: none;">
        <tr>
            <td class="td-center" colspan="2">
               ...
               <input class="input input-xsmall-onnet" type="text" id="balance" name="balance" maxlength="5" size="5" value="{{ notify["b_limit"] }}" /> {_ rub _}.
                {% button id="notify_disable_btn" class="btn btn-xs btn-onnet pull-right" text=_"disable alert"
                    action={postback postback="notify_disable_btn" delegate="onnet"}
                %}
                {% button id="notify_submit_btn" class="btn btn-xs btn-onnet pull-right" text=_"save"
                    action={postback postback="notify_submit_btn" delegate="onnet" qarg="balance" }
                %}
            </td>
        </tr>
    </tbody>
