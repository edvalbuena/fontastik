...
<table class="table table-condensed table-hover table-centered">
    ...
    <tbody>
        ...
        {% endif %}  
        {% for amount, date, comment, status, cancel_date in m.onnet[{account_payments limit=1}] %}
            <tr>
              <td>{_ Previous payment _}</td>
              <td>{{ date }} - {{ amount }} {_ rub. _} <span class="onnet-07em">({_ excl VAT _})</span> - 
                  {% if status=="2" %}
                    {_ Chargeback _} - {{ cancel_date }}
                  {% else %}
                    {% if comment|match:"ssist" %}
                      {_ ASSIST _}
                    {% elseif comment|match:"DengiOnl" %}
                      {_ Dengi Online _}
                    {% elseif comment|match:"Yandex.Money" %}
                      {_ Yandex.Money _}
                    {% else %}
                      {_ Wire transfer _}
                    {% endif %}
